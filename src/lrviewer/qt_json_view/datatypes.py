...
class DataType(object):
    ...
    # TODO Use color fore/background
    # COLOR = QtCore.Qt.GlobalColor.black
    # INACTIVE_COLOR = QtCore.Qt.GlobalColor.gray
...
class BoolType(DataType):
    ...
    def clicked(self, parent, index, pos, rect):
        ...
        if pos.x() - rect.x() < 18:
            item.setData(
                not item.data(QtCore.Qt.ItemDataRole.DisplayRole),
...
class DictType(DataType):
    ...
    def value_item(self, value, model, key):
        ...
        item.setData(font, QtCore.Qt.ItemDataRole.FontRole)
        item.setData(
            QtGui.QBrush(QtCore.Qt.GlobalColor.gray),
            # QtGui.QBrush(QtCore.Qt.GlobalColor.lightGray),
            QtCore.Qt.ItemDataRole.ForegroundRole,
        )
        item.setFlags(
...
class UrlType(DataType):
    ...
    def actions(self, index):
        ...
        explore = QtGui.QAction("Explore ...", None)
    ...
    def value_item(self, value, model, key=None):
        ...
        icon = (
            QtWidgets.QApplication.instance().style()
            # .standardIcon(getattr(QStyle.StandardPixmap, "SP_DriveNetIcon"))
            .standardIcon(QStyle.StandardPixmap.SP_DriveNetIcon)
            # .standardIcon(QtWidgets.QApplication.instance().style().SP_DriveNetIcon)
        )
...
class FilepathType(UrlType):
    ...
    def actions(self, index):
        actions = super(UrlType, self).actions(index)
        explore_path = QtGui.QAction("Explore Path ...", None)
        actions.append(explore_path)
        path = index.data(QtCore.Qt.ItemDataRole.DisplayRole)
        if os.path.isfile(path):
            open_file = QtGui.QAction("Open File ...", None)
    ...
    def value_item(self, value, model, key=None):
        ...
        #         .style()
        #         .standardIcon(QtWidgets.QApplication.instance().style().SP_FileIcon)
        #     )
        # elif os.path.isdir(value):
        #     icon = (
        #         QtWidgets.QApplication.instance()
        #         .style()
        #         .standardIcon(QtWidgets.QApplication.instance().style().SP_DirIcon)
        #     )
        # else:
        #     return item
        # item.setData(icon, QtCore.Qt.ItemDataRole.DecorationRole)
